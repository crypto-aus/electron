...
ipcMain.handle('ELECTRON_BROWSER_DESKTOP_CAPTURER_GET_SOURCES', (event, captureWindow, captureScreen, thumbnailSize, fetchWindowIcons) => {
...
desktopCapturer.emit = (event, name, sources, fetchWindowIcons) => {
  ...
  if (handledRequest.deferred) {
    handledRequest.deferred.resolve(result)
  }
